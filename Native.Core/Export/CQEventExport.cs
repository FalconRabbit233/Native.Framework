...
namespace Native.App.Export
{
	...
	{	
		...
		{	
			return "9,xyz.presidium.dicebot";	
		}	
		...
		{	
			...
			Type appDataType = typeof (AppData);	
			// 注册一个 CQApi 实例	
			AppInfo appInfo = new AppInfo ("xyz.presidium.dicebot", 1, 9, "骰子机2020", "1.0.0", 1, "uranus_r", "骰子机2020描述", authCode);	
			appDataType.GetRuntimeProperty ("CQApi").GetSetMethod (true).Invoke (null, new object[] { new CQApi (appInfo) });	
			AppData.UnityContainer.RegisterInstance<CQApi> ("xyz.presidium.dicebot", AppData.CQApi);	
			// 向容器注册一个 CQLog 实例	
			appDataType.GetRuntimeProperty ("CQLog").GetSetMethod (true).Invoke (null, new object[] { new CQLog (authCode) });	
			AppData.UnityContainer.RegisterInstance<CQLog> ("xyz.presidium.dicebot", AppData.CQLog);	
